         ...
         try {
             ...
             if (data.success && data.resident) {
                 ...
                 if(r.basic) setBasic({
                   name: r.basic.name || '',
                   email: r.basic.email || '',
                   phone_number: r.basic.phone_number || '',
                 });
                 if(r.flat) setFlat({
                   block_name: r.flat.block_name || '',
                   floor: r.flat.floor || '',
                   flat_number: r.flat.flat_number || '',
                   flat_type: r.flat.flat_type || '',
                   occupancy_type: r.flat.occupancy_type || 'Owner',
                   access_role: r.flat.access_role || 'Primary',
                   move_in_date: r.flat.move_in_date || '',
                   move_out_date: r.flat.move_out_date || '',
                 });
                 if(r.identity) setIdentity({
                   id_type: r.identity.id_type || 'Aadhaar',
                   id_number: r.identity.id_number || '',
                   id_proof_url: r.identity.id_proof_url || '',
                 });
                 if(r.emergency) setEmergency({
                   emergency_name: r.emergency.emergency_name || '',
                   emergency_relation: r.emergency.emergency_relation || '',
                   emergency_phone: r.emergency.emergency_phone || '',
                 });
                 if(r.notifications) setNotifications(r.notifications);
                 if(r.access) setPermissions(r.access);
                 if(r.vehicles?.length) setVehicles(r.vehicles.map((vehicle: { vehicle_type?: string; vehicle_number?: string; parking_slot?: string }) => ({
                   vehicle_type: vehicle.vehicle_type || 'Car',
                   vehicle_number: vehicle.vehicle_number || '',
                   parking_slot: vehicle.parking_slot || '',
                 })));
                 if(r.family?.length) setFamily(r.family.map((member: { name?: string; age?: string | number | null; relation?: string | null; phone?: string | null; phone_number?: string | null }) => ({
                   name: member.name || '',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-20">
      <div>
        ...
        <p className="text-slate-500 mt-2">Update all details including structural mapping, identity, vehicles, and permissions.</p>
      </div>
