...
export type ImportantContactPerson = {
  id: number;
  name: string;
  phone_number: string;
  role?: string;
...
export type ImportantServiceStaff = {
  id: number;
  name: string;
  phone_number: string;
  type: 'Security' | 'Cleaner' | 'Plumber' | 'Electrician';
...
export type ResidentImportantContacts = {
  admins: ImportantContactPerson[];
  managers: ImportantContactPerson[];
  service_staff: ImportantServiceStaff[];
};
...
export type Invoice = {
  ...
  billing_custom_amount?: number | null;
  amount: number;
  month_year: string;
  status: 'Paid' | 'Unpaid' | 'Overdue' | 'PartiallyPaid' | 'Waived';
  ...
  notes?: string;
  pdf_url?: string;
  line_items?: Array<{ id: number; label: string; amount: number; calculation_mode: string; sort_order: number }>;
  payments?: Array<{ id: number; amount: number; payment_method: string; payment_gateway: string; payment_reference: string; status: string; paid_at: string | null; notes: string }>;
