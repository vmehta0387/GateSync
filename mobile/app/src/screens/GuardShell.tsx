...
import { GuardIncidentsScreen } from './GuardIncidentsScreen';
import { GuardProfileScreen } from './GuardProfileScreen';
import { GuardVisitorsScreen } from './GuardVisitorsScreen';
...
export function GuardShell() {
  ...
  const approvedArrivals = useMemo(
  ...
  return (
    <SafeAreaView edges={['top', 'left', 'right']} style={styles.safeArea}>
      ...
      >
        ...
        {activeTab === 'incidents' ? (
          <GuardIncidentsScreen incidents={incidents} onSubmit={(payload) => withReload(() => createIncident(payload))} />
        ) : null}

        {activeTab === 'profile' ? <GuardProfileScreen /> : null}
      </ScrollView>
    </SafeAreaView>
  );
}
